...
export default function MediaPage({ id }: { id: string }) {
  ...
  const share = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: media?.title ?? 'Aura media', text: media?.description || undefined, url });
