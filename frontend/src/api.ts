...
export const api = {
  ...
  register: (name: string, email: string, password: string, phoneNumber?: string) => request<MessageResponse>('/auth/register', { method: 'POST', body: JSON.stringify({ name, email, password, phoneNumber: phoneNumber || null }) }),
