...
public sealed class MediaController(IMediaService media, IUploadService uploads) : ControllerBase
{
    ...
    [Authorize, HttpPost("uploads")] public async Task<ActionResult<UploadResponse>> BeginUpload(UploadRequest request) => Ok(await uploads.BeginAsync(request, User.UserId(), RequestMetadataExtractor.Extract(HttpContext)));
