...
public sealed class UploadService(IMediaRepository media, IUserRepository users, IConfigurationService configuration, IMediaStorage storage, IClock clock) : IUploadService
{
    public async Task<UploadResponse> BeginAsync(UploadRequest request, string userId, ClientMetadata metadata)
    {
        ...
        var item = new MediaDocument { OwnerId = user.Id!, OwnerName = user.Name, Title = string.IsNullOrWhiteSpace(request.Title) ? clock.UtcNow.ToString("yyyy-MM-dd hh mm ss") : request.Title.Trim(), Description = request.Description, Tags = request.Tags?.Select(x => x.Trim().ToLowerInvariant()).Where(x => x.Length > 0).Distinct().ToList() ?? [], MediaType = request.ContentType.Split('/')[0], ContentType = request.ContentType, ObjectKey = key, UploadMetadata = metadata, CreatedAt = clock.UtcNow };
