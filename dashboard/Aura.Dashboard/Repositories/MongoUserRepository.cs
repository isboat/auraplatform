...
public sealed class MongoUserRepository(MongoContext context) : IUserRepository
{
    ...
    public async Task SaveAsync(StaffUser u) { if (u.Id is null) await context.StaffUsers.InsertOneAsync(u); else await context.StaffUsers.ReplaceOneAsync(x => x.Id == u.Id, u, new ReplaceOptions { IsUpsert = false }); }
    public Task DeleteAsync(string id) => context.StaffUsers.DeleteOneAsync(x => x.Id == id);
    public async Task<bool> AcceptInvitationAsync(string tokenHash, DateTime now, string passwordHash)
    {
        var result = await context.StaffUsers.UpdateOneAsync(
            user => user.InvitationTokenHash == tokenHash && user.InvitationExpiresAtUtc > now && !user.IsBlocked,
            Builders<StaffUser>.Update
                .Set(user => user.PasswordHash, passwordHash)
                .Set(user => user.EmailVerified, true)
                .Set(user => user.InvitationTokenHash, null)
                .Set(user => user.InvitationExpiresAtUtc, null)
    ...
    {
        var result = await context.StaffUsers.UpdateOneAsync(
            user => user.PasswordResetTokenHash == tokenHash && user.PasswordResetExpiresAtUtc > now && !user.IsBlocked,
            Builders<StaffUser>.Update
                .Set(user => user.PasswordHash, passwordHash)
                .Set(user => user.PasswordResetTokenHash, null)
                .Set(user => user.PasswordResetExpiresAtUtc, null)
                .Inc(user => user.SessionVersion, 1));
        return result.ModifiedCount == 1;
    }
