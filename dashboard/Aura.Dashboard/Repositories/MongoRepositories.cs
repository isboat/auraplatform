...
public sealed class MongoContext(IConfiguration configuration)
{
    ...
    public IMongoCollection<StaffUser> Users => _db.GetCollection<StaffUser>("users");
    public IMongoCollection<ManagedMedia> Media => _db.GetCollection<ManagedMedia>("media");
    public IMongoCollection<AuditEvent> Audit => _db.GetCollection<AuditEvent>("managementAudit");
}
...
{
    public Task<StaffUser?> FindAsync(string id)=>context.Users.Find(x=>x.Id==id).FirstOrDefaultAsync(); public Task<StaffUser?> FindByEmailAsync(string email)=>context.Users.Find(x=>x.Email==email.ToLowerInvariant()).FirstOrDefaultAsync();
    public async Task<PageResult<StaffUser>> SearchAsync(string? q,string? role,bool? blocked,int page,int size){var f=Builders<StaffUser>.Filter;var fs=new List<FilterDefinition<StaffUser>>();if(!string.IsNullOrWhiteSpace(q))fs.Add(f.Or(f.Regex(x=>x.Name,new(q,"i")),f.Regex(x=>x.Email,new(q,"i"))));if(!string.IsNullOrWhiteSpace(role))fs.Add(role==Roles.Administrator?f.Or(f.Eq(x=>x.IsAdministrator,true),f.AnyEq(x=>x.Roles,role)):f.AnyEq(x=>x.Roles,role));if(blocked.HasValue)fs.Add(f.Eq(x=>x.IsBlocked,blocked));var filter=fs.Count==0?f.Empty:f.And(fs);var total=await context.Users.CountDocumentsAsync(filter);var items=await context.Users.Find(filter).SortBy(x=>x.Name).Skip((page-1)*size).Limit(size).ToListAsync();return new(items,total,page,size);}
    public async Task SaveAsync(StaffUser u){if(u.Id is null)await context.Users.InsertOneAsync(u);else await context.Users.ReplaceOneAsync(x=>x.Id==u.Id,u,new ReplaceOptions{IsUpsert=false});} public Task DeleteAsync(string id)=>context.Users.DeleteOneAsync(x=>x.Id==id);
    public async Task<Dictionary<string,long>> CountsAsync(){var all=await context.Users.Find(_=>true).ToListAsync();return new(){{"Total",all.Count},{"Verified",all.Count(x=>x.EmailVerified)},{"Blocked",all.Count(x=>x.IsBlocked)},{"Reviewers",all.Count(x=>x.IsReviewer&&!x.IsAdmin)},{"Administrators",all.Count(x=>x.IsAdmin)}};}
}
